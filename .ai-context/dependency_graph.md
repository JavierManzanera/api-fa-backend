...
| OBJ-013 | Harden `enforce_rate_limit` keying (finding #17): AND-combined `(scope,ip,email)` is trivially resettable by rotating either field | solution-architect → qa-engineer → developer → qa-engineer ∥ security-specialist | **Gate 3 PASS both** (qa `a776433`, security `8130bfe`), finding #17 **CERRADO**, merged into integration branch (`8497a6b`). Fix itself surfaced 2 new findings, see OBJ-014 below | audit-report.md §Gate 3 OBJ-009 (finding #17) + §Gate 3 OBJ-013; tests=docs/testing/obj-013-test-report.md |
| OBJ-014 | Mitigate finding #20 (NEW, MEDIUM — OBJ-013's ip/email decoupling enables a targeted single-victim email-lockout DoS); bundle finding #21 (LOW — `RATE_LIMIT_IP_MULTIPLIER` needs a `>=1` validator) | solution-architect → developer ∥ database-architect (index) → qa-engineer ∥ security-specialist | Gate 1 (`16561e7`) + implementation (`9b4e378`, 325 passed/0 failed) + index migration (`632e772`) all done, merged into integration branch (`5b5f34b`). Note for Gate 3: implementation correctly widened 8 pre-existing single-IP regression tests to `limit-1` (documented in design doc §9 — expected consequence of reserving 1 slot, not a bug) — Gate 3 should double-check this reasoning, not just the green result. Gate 3 (qa-engineer ∥ security-specialist) not yet dispatched | audit-report.md §Gate 3 OBJ-013 (findings #20, #21); design=docs/api/obj-014-design-notes.md |
