...
| OBJ-014 | Mitigate finding #20 (NEW, MEDIUM — OBJ-013's ip/email decoupling enables a targeted single-victim email-lockout DoS); bundle finding #21 (LOW — `RATE_LIMIT_IP_MULTIPLIER` needs a `>=1` validator) | solution-architect → developer ∥ database-architect (index) → qa-engineer ∥ security-specialist | Gate 1 done (`16561e7`, merged), design = "reserved fresh-IP slot" pool inside the email-keyed check, `RATE_LIMIT_EMAIL_RESERVED_SLOTS` default 1 (flagged as a tunable Gate-1 tradeoff). developer + database-architect (index widening) dispatched in parallel | audit-report.md §Gate 3 OBJ-013 (findings #20, #21); design=docs/api/obj-014-design-notes.md |
