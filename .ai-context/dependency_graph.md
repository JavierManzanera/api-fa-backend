...
| OBJ-014 | Mitigate finding #20 (NEW, MEDIUM — OBJ-013's ip/email decoupling enables a targeted single-victim email-lockout DoS: attacker from own IP, within own budget, exhausts one chosen victim's shared email-keyed counter, blocking that victim's real requests too, on all 6 endpoints); bundle finding #21 (LOW — `RATE_LIMIT_IP_MULTIPLIER` has no `>=1` validator, a `0`/negative value self-DoSes all 6 endpoints) | solution-architect → developer → qa-engineer ∥ security-specialist | User explicitly chose "fix now, same batch" over "track and close batch as-is" (2026-08-25) — dispatched, in progress | audit-report.md §Gate 3 OBJ-013 (findings #20, #21) |
