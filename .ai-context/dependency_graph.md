...
| OBJ-012 | Real TLS-terminated-Postgres DAST check (sandbox never had certs to verify `require`/`verify-full` against); DAST pass surfaced new finding #18 (no private-CA pinning for `verify-full`), fixed same objective | devops-engineer → security-specialist; finding #18 fix: developer → security-specialist | devops-engineer DAST (`f3fa924`) + finding #18 fix (`fe2e0c5`, `POSTGRES_SSL_ROOT_CERT`) done, merged into integration branch; security-specialist Gate 3 review (covering both) not yet dispatched | obj-003-design-notes.md residual note; obj-012-tls-dast-verification.md §7 (finding #18) |
| OBJ-013 | Harden `enforce_rate_limit` keying (finding #17): AND-combined `(scope,ip,email)` is trivially resettable by rotating either field | solution-architect → qa-engineer → developer → qa-engineer ∥ security-specialist | Gate 1 done (`a7db0ef`, merged `bbe18f5`); Gate 2 red-phase tests dispatched to qa-engineer, in progress | audit-report.md §Gate 3 OBJ-009 (finding #17) |
